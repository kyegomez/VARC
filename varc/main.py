...
class GroupedQueryAttention(nn.Module):
    ...
    def forward(
        ...
        q = q.view(B, N, self.n_query_heads, self.d_head).transpose(1, 2)  # (B, n_q_heads, N, d_head)
        k = k.view(B, N, self.n_kv_heads, self.d_head).transpose(1, 2)  # (B, n_kv_heads, N, d_head)
        v = v.view(B, N, self.n_kv_heads, self.d_head).transpose(1, 2)  # (B, n_kv_heads, N, d_head)
        ...
        attn_scores = torch.matmul(q, k.transpose(-2, -1)) * self.head_scale  # (B, n_q_heads, N, N)
